...
- `brel tag` creates and pushes a release tag, either from explicit `--tag`/`--target` args or from a GitHub merged-PR event.
...
- `brel release-pr` requires `gh` (GitHub CLI).
- `brel release-pr` requires a GitHub token in `GH_TOKEN` or `GITHUB_TOKEN`.
...
- If the merged PR is managed by `brel` and titled `Release <rendered-tag>`, the workflow runs `brel tag`.
